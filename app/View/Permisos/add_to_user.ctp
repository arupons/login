...
<?php echo $this->Form->end(__('Ingresar'));?>
		...
		<?php echo $this->Html->link(__('<i class="icon-arrow-left"></i> Volver'), array('controller'=>'users','action' => 'index'), array('class' => 'btn btn-primary', 'escape' => false)); ?>
                ...
                if(typeof(id) != "undefined" && id !== null)
                {
                    if ( $('#Permiso'+id+'Id').length ) {
                        ...
                        $('#datas').append('<div id="fields'+id+'"><input name="data[Permiso]['+id+'][menu_id]" id="Permiso'+id+'MenuId" type="hidden" value='+id+'>'
                            +'<input name="data[Permiso]['+id+'][model]" id="Permiso'+id+'Model" type="hidden" value="User">'
                            +'<input name="data[Permiso]['+id+'][foreign_key]" id="Permiso'+id+'ForeignKey" type="hidden" value='+user+'>'
...
{
	echo "<h1>El usuario seleccionado no es válido o no existe, seleccione un usuario válido e intente de nuevo.</h1>";
}